...
Blockly.Arduino['procedures_callreturn'] = function (block) {
    ...
    for (var i = 0; i < block.arguments_.length; i++) {
        args[i] = Blockly.Arduino.valueToCode(block, 'ARG' + i,
                Blockly.Arduino.ORDER_COMMA) || '0';  // default value as '0' for arguments not passed to function
    }
    var code = funcName + '(' + args.join(', ') + ')';
    return [code, Blockly.Arduino.ORDER_UNARY_POSTFIX];  // Fix: "Uncaught TypeError: Expecting valid order from value block: procedures_callreturn"
};
...
Blockly.Arduino['procedures_ifreturn'] = function (block) {
    ...
    if (block.hasReturnValue_) {
        var value = Blockly.Arduino.valueToCode(block, 'VALUE',
                Blockly.Arduino.ORDER_NONE) || '0';  // default value as '0' for arguments not passed to function
