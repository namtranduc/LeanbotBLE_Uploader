...
Code.copyToClipboard = function () {
    ...
    // document.execCommand("copy");
        ...
        navigator.clipboard.writeText(Blockly.Arduino.workspaceToCode(Code.workspace))
                .then(() => {console.log('Code copied!') })
                .catch((error) => { console.log('Copy failed! ${error}') });
    }
};

Code.copyToClipboardToNewWindow = function () {
    //var encodetext = btoa(document.getElementsByClassName("ace_content")[0].innerText)
    var encodetext1 = btoa(Blockly.Arduino.workspaceToCode(Code.workspace))
    navigator.clipboard.writeText(Blockly.Arduino.workspaceToCode(Code.workspace))
        .then(() => {console.log('Code copied!') })
        .catch((error) => { console.log('Copy failed! ${error}') });
    localStorage.setItem("codeText", encodetext1);
    var currentFile = localStorage.getItem("currentFile")
    localStorage.setItem("presentFile",currentFile)
    navigator.clipboard.readText().then(
        text => window.open(
            conf.URL_BLOCKLY_CODE.url, '_blank'
         )
    );
    console.log(Blockly.Arduino.workspaceToCode(Code.workspace))
    //console.log(window.parent.location);
};

/**
 * Creates an XML string containing the blocks from the Blockly workspace and copies it to clipboard system
 */
Code.copyXMLToClipboard = function () {
    var xmlData = Blockly.Xml.workspaceToDom(Code.workspace);
    var dataToCopy = Blockly.Xml.domToPrettyText(xmlData);
...
/**
 * Parses the XML from the system clipboard and replace the blocks in the Blockly workspace.
 */
Code.pasteXMLFromClipboard = function () {
    // var clipText;
    // navigator.clipboard.readText().then(
 ...
 * Load the demo file.
 */
Code.loadDemoFile = function () {
    var request = new XMLHttpRequest();
    request.open('GET', "examples/BlocklyDemo.xml", true);
    request.send(null);
    request.onreadystatechange = function () {
        // console.log( request.readyState );
        // console.log( request.status );
        if (request.readyState === 4 && request.status === 200) {  // ready = 4 (DONE), status = 200 (OK)
            var type = request.getResponseHeader('Content-Type');
            if (type.indexOf("text") !== 1) {
                // console.log( request.responseText );
                Code.loadBlocksfromXml(request.responseText);
                Code.workspace.render();
            }
        }
    }
