...
Code.loadBlocks = function () {
    const defaultXml = `
      <xml xmlns="https://developers.google.com/blockly/xml">
        <block type="Leanbot.whenStarted" id="j6T+-bX4ek*WN[:xS]o|" x="213" y="13">
          <next>
            <block type="LbMission.beginByTeacher_end" id="kB8SuvVJYB;LAxKzYaGS">
              <field name="comboTouch">TB1A + TB1B</field>
            </block>
          </next>
        </block>
      </xml>
    ...
    if (loadOnce) {
        ...
        if ( ! loadOnce.includes("</block>") ) {    // use default if workspace is empty (first open)
            loadOnce = defaultXml;
        }
...
//        blocklyDiv.style.top = metrics.absoluteTop + 'px';
    ...
    // Code.workspace.registerButtonCallback('createVarBtnGlobal', createVarBtnGlobalCallBack);
    ...
    Code.workspace.registerToolboxCategoryCallback('VARIABLE_TYPED', globalVariablesCallBack);
    Code.workspace.registerToolboxCategoryCallback('VARIABLE_TYPED_NUM', numVariablesCallBack);
    Code.workspace.registerToolboxCategoryCallback('VARIABLE_TYPED_LOCAL', localVariablesCallBack);
        ...
        function onMouseMove(e) {
            ...
            if (direction === "H") // Horizontal
            {
                ...
                if (document.getElementById("content_code").offsetWidth < 50){
                    ...
                    document.getElementById("copyCodeButton1").style.visibility = 'hidden';
                }else{
                    document.getElementById("copyCodeButton").style.visibility = 'visible';
                    document.getElementById("copyCodeButton1").style.visibility = 'visible';
                }
            }
    ...
    Code.changeRenderingConstant(11);  // set default font size
...
Code.initLanguage = function () {
    ...
    document.getElementById('loadDemoButton').title = MSG['loadDemoButton_span'];
    document.getElementById('copyXMLButton').title = MSG['copyXMLButton_span'];
    document.getElementById('pasteXMLButton').title = MSG['pasteXMLButton_span'];
...
window.addEventListener('paste', (event) => {
  ...
  // verify Xml string
  var doc = Blockly.utils.xml.textToDomDocument(pasteText);
  ...
  // generate workspace
  Code.loadBlocksfromXml(pasteText);
