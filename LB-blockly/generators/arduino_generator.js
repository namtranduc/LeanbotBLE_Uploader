 ...
 * @type string
 */
Blockly.Arduino.LeanbotBlocklyVersion = "Leanbot Blockly Editor 2026-02-07";
...
Blockly.Arduino.finish = function (code) {
    ...
    var info = Blockly.Arduino.generateInfo();
    var allDefs = includes.join('\n') + definitions.join('\n') + variables.join('\n') + functions.join('\n');
    var setup = 'void setup() {' + setups.join('\n  ') + '\n}\n\n';

    var loop = 'void loop() {\n  ' + code.replace(/\n/g, '\n  ');
    loop  = loop.slice(0, -2);  // remove last 2 whitespaces
    loop += '}'

    return info + allDefs + setup + loop;
};


/**
 * Generate information message
 * @return {string} Arduino code with comments.
 */
Blockly.Arduino.generateInfo = function () {
  const now           = new Date();
  const offsetMs      = now.getTimezoneOffset() * 60 * 1000;
  const dateLocal     = new Date(now.getTime() - offsetMs);
  const generatedTime = dateLocal.toISOString().slice(0, 19).replace("T", " ");

  var infoMsg = "";
  infoMsg += "//  Generated Leanbot Arduino code"                + "\n";
  infoMsg += "//    by " + Blockly.Arduino.LeanbotBlocklyVersion + "\n";
  infoMsg += "//    at " + generatedTime                         + "\n";
  infoMsg += "\n";
